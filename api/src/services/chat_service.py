...
class ChatService:
    def __init__(self, company_name, company_site):
        ...
        self.data = pd.read_csv(
            "data/embeddings/" + urlparse(self.company_site).netloc + ".csv",
            index_col=0,
        )
        self.data["embeddings"] = (
            self.data["embeddings"].apply(literal_eval).apply(np.array)
        )
        self.data.head()
        self.max_tokens = 500
    ...
    def create_context(self, question, max_len):
        ...
        self.data["distances"] = distances_from_embeddings(
            q_embeddings, self.data["embeddings"].values, distance_metric="cosine"
        )
        ...
        message = [
            {
                ...
                "your knowledge and do not answer questions outside this scope.\n\n",
            },
